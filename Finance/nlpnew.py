...
def nlp(text,comp,name):
    ...
    def hasAlphanumeric(x):
        ...
        if re.search(alphanumerreg, x):
            try:
                found = re.search(alphanumerreg, x).group()
                return found
    ...
    def isInvoiceDate(x):
        ...
        if re.search(dateregex1, x):
            try:
                found = re.search(dateregex1, x).group()
                return found
            except AttributeError:
                return ''
        if re.search(dateregex2, x):
            try:
                found = re.search(dateregex2, x).group()
                return found
            except AttributeError:
                return ''
        if re.search(dateregex3, x):
            try:
                found = re.search(dateregex3, x).group()
                return found
            except AttributeError:
                return ''
        if re.search(dateregex4, x):
            try:
                found = re.search(dateregex4, x).group()
                return found
            except AttributeError:
                return ''
        if re.search(dateregex5, x):
            try:
                found = re.search(dateregex5, x).group()
                return found
            except AttributeError:
                return ''
        return ''
    ...
    def hasAmountAtEnd(x):
        amountregex = r'\d+.\d{2}'
        if re.search(amountregex, x):
            try:
                found=re.search(amountregex,x).group()
                return found
            except AttributeError:
                return ''
        return ''
    ...
    for lineno in range(len(info)):
        ...
        if tablecol == False:
            ...
            if interest_count[1] > 0:
                if hasAmountAtEnd(line)!='':
                    usefuldata["Total Bill"] = hasAmountAtEnd(line) #tokensinline[len(tokensinline) - 1]

            elif interest_count[0] > 0:
                if interest_count[0] >= interest_count[2]:
                    if hasAmountAtEnd(line)!='': #(tokensinline[len(tokensinline) - 1]):
                        usefuldata["Invoice Number"] = hasAmountAtEnd(line)#tokensinline[len(tokensinline) - 1]
                    #else:
